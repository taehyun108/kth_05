...
**현재 진행 상태는 `STATUS.md`를 먼저 볼 것.** (완료 단계·대기 항목·다음 할 일·리허설 결과)
작업 전 해당 문서를 읽을 것. CLAUDE.md는 규칙만, 상세는 docs/에 있다.

| 작업 | 문서 |
|---|---|
| **진행 상태·다음 할 일** | `STATUS.md` |
| **Z2 외부망 실행 순서** | `docs/Z2-RUNBOOK.md` |
...
| 6 | **L1·L2·L3는 선택 스테이지. 발행은 항상 L0로, 발송은 채널 포맷 충족 시에만.** 웹·메일은 L0 추출 요약으로도 발행한다. **카톡은 고정 포맷(존댓말 평서문)을 만족하는 건만 발송** — L0 추출 요약(`summary_method=extractive`)만 있는 건은 카톡 스킵하고 운영 알림에 건수 기록 | `--only L0` 발행 + 카톡 extractive 스킵 테스트 |
